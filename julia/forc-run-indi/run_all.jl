...
function run_RDF(h, h_lag, J, J_cv, level, head_line, Y, Y_lag, X_lag, X_lag2)
    ...
    m_vec = 1:1:25
    n_est = 50
    m, ŷ, RMSE_cv = rfr_oos_forc(J , h, h_lag, J_cv, Y, Y_lag, X_lag, m_vec, n_est)
...
function run_TRD(h, h_lag, J, J_cv, level, head_line, Y, Y_lag, X_lag, X_lag2)
    ...
    r = 1
    @time ŷ, RMSE = dfm_oos_forc(J, h, h_lag, Y, Y_lag, X_lag, r)
    println("Dynamic Factor r = $r: \nRMSE = $RMSE")
    df[:DFM] = ŷ

    #Forecast using AR1 model #
    @time ŷ, RMSE = AR_oos_forc(J, h, h_lag, Y, Y_lag)
    println("AR(1) Model: \nRMSE = $RMSE")
    df[:ARM] =  ŷ

    #Forecast using OLS with Lagged Aggregate Variable
    @time ŷ, RMSE = OLS_oos_forc(J, h, h_lag, Y, Y_lag, X_lag)
    println("OLS Regression Including all Variables: \nRMSE = $RMSE")
    df[:OLS] = ŷ

    #Model Average out of sample estimation
    Q = 1
    @time ŷ, RMSE = modelavg_oos_forc(J, h, h_lag, Y, Y_lag, X_lag, Q)
    ...
    @time ŷ, RMSE = rwm_oos_forc(J, h, h_lag, Y, Y_lag)
...
## Prepare for the matrices used for forecasting
level = 4
h = 3
J = 301
h, h_lag, J, J_cv, level, head_line, Y, Y_lag, X_lag, X_lag2 = prepare(level, h, J)
run_BMA(h, h_lag, J, J_cv, level, head_line, Y, Y_lag, X_lag, X_lag2)
#run_BMA2(h, h_lag, J, J_cv, level, head_line, Y, Y_lag, X_lag, X_lag2)
#run_DFM2(h, h_lag, J, J_cv, level, head_line, Y, Y_lag, X_lag, X_lag2)
#run_LAS(h, h_lag, J, J_cv, level, head_line, Y, Y_lag, X_lag, X_lag2)
#run_LAS2(h, h_lag, J, J_cv, level, head_line, Y, Y_lag, X_lag, X_lag2)
#run_RDG(h, h_lag, J, J_cv, level, head_line, Y, Y_lag, X_lag, X_lag2)
#run_RDG2(h, h_lag, J, J_cv, level, head_line, Y, Y_lag, X_lag, X_lag2)
#run_RDF(h, h_lag, J, J_cv, level, head_line, Y, Y_lag, X_lag, X_lag2)
#run_RDF2(h, h_lag, J, J_cv, level, head_line, Y, Y_lag, X_lag, X_lag2)
#run_TRD(h, h_lag, J, J_cv, level, head_line, Y, Y_lag, X_lag, X_lag2)
